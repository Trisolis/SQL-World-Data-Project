...
JOIN indicators i ON c.iso_code=i.iso_code
WHERE birth_rate = (
    SELECT MAX(i2.birth_rate)
    FROM countries c2 
    JOIN indicators i2 ON c2.iso_code=i2.iso_code
    WHERE c2.region = c.region
)
...
FROM countries co
JOIN cities ci ON co.iso_code=ci.iso2
WHERE (
    SELECT COUNT(*)
    FROM countries co2 
    JOIN cities ci2 ON co2.iso_code=ci2.iso2
...
END AS hdi_tier
FROM cities c 
JOIN indicators i ON c.iso2=i.iso_code
...
FROM countries c
JOIN economy e ON c.iso_code=e.iso_code
WHERE gdp > (SELECT SUM(gdp) FROM countries c2 JOIN economy e2 ON c2.iso_code=e2.iso_code WHERE region = 'Africa')
ORDER BY gdp DESC;
